...
class Plucky(AlwaysInFrontCharacter):
    ...
    framesDir = join(DIR, 'pluckyFrames')
    ...
    def __init__(self):
        ...
        # Load all the frames
        self.pixmaps = {}
        for frame in os.listdir(self.framesDir):
            self.pixmaps[frame[:-4]] = QPixmap(join(self.framesDir, frame))
    ...
    def move(self):
        self.setGeometry(self.x() + self.dx, self.y() + self.dy, self.width(), self.height())
